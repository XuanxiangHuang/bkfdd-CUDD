...
program="./bkfdds -autodyn -automethod bkfddgroupnmeg -reordering bkfdd-group-nmeg-mix -drop Benchmark/"
arguments="-davioexist 100"
cat $test_cases_file | while read oneline
do
    timeout 3600 "$program$oneline $arguments -dumpfile BKFDD_group/$oneline -dumpblif" >> SND_BKFDDgroup.txt
done
...
if [ "$oneline" == "ITC_b15.blif" ] ; then
		...
		arguments="-davioexist 25 -choosenew 9998 -choosedav 9949 -choosefail 100"
fi
    timeout 3600 "$program$oneline $arguments -dumpfile BKFDD_group/$oneline -dumpblif" >> SND_BKFDDgroup.txt
done
