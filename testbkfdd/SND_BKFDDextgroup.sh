...
program="./bkfdds -autodyn -automethod bkfddgroup -reordering bkfdd-group-mix -drop -groupcheck bkfcheck2 Benchmark/"
arguments="-davioexist 100"
cat $test_cases_file | while read oneline
do
    timeout 3600 "$program$oneline $arguments -dumpfile BKFDD_extgroup/$oneline -dumpblif" >> SND_BKFDDextgroup.txt
done
