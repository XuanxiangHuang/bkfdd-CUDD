...
program="./bkfdds -autodyn -automethod oet -reordering oet -drop Benchmark/"
arguments="-davioexist 100"
cat $test_cases_file | while read oneline
do
    timeout 3600 "$program$oneline $arguments -dumpfile BKFDD_oet/$oneline -dumpblif" >> SND_BKFDDoet.txt
done
